...
# --- RESULT ---
    ...
    if score >= 70: st.success(f"Score: {score}% — Congratulations! You have mastered {st.session_state.topic}.")
    else: st.error(f"Current Score: {score}% — Let's reinforce the concept with a Feynman explanation")
    ...
    if score >= 70:
        ...
        if col1.button("MASTER NEW TOPIC", use_container_width=True): 
