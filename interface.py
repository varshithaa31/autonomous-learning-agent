...
st.markdown("""
    ...
    .stButton>button { 
        ...
        border-radius: 14px; font-weight: 800; padding: 0.8rem 2rem;
    }
...
def draw_sidebar():
    with st.sidebar:
        ...
        if st.button("Logout", use_container_width=True): 
            st.session_state.view = "Auth"; st.session_state.user = None; st.rerun()
...
if st.session_state.view == "Auth":
    ...
    draw_sidebar()
    if st.session_state.agent_data and "explanation" in st.session_state.agent_data:
        ...
        st.error("Session data lost. Please restart from Dashboard.")
        if st.button("Return to Dashboard"): st.session_state.view = "Dashboard"; st.rerun()

elif st.session_state.view == "Quiz":
    draw_sidebar()
    if st.session_state.agent_data:
        quiz = st.session_state.agent_data['quiz_data']
        st.markdown(f"<h1 class='main-header'>Assessment</h1>", unsafe_allow_html=True)
        with st.form("quiz_form"):
            ...
            if st.form_submit_button("SUBMIT"):
                ...
                st.rerun()
    else: st.session_state.view = "Dashboard"; st.rerun()
    ...
    else: st.error(f"Score: {score}%. Let's try the Feynman approach.")
    
    for i, q in enumerate(quiz):
        status = "correct-card" if u_ans[i][0] == q['a'] else "wrong-card"
        st.markdown(f"<div class='feedback-card {status}'><b>{i+1}. {q['q']}</b><br>Correct: {q['a']}</div>", unsafe_allow_html=True)
    
    if score >= 70:
        if st.button("NEW TOPIC"): st.session_state.view = "Dashboard"; st.rerun()
    else:
        if st.button("FEYNMAN ANALOGY"): st.session_state.view = "Feynman"; st.rerun()

elif st.session_state.view == "Feynman":
    draw_sidebar()
    if st.session_state.agent_data and "feynman_text" in st.session_state.agent_data:
